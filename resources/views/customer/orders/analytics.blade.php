...
<div class="space-y-8">
    <section class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <article class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <p class="text-xs uppercase tracking-[0.2em] text-slate-500">Total Orders</p>
            <p class="mt-3 text-3xl font-semibold text-slate-900">{{ $totalOrders }}</p>
        </article>

        <article class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <p class="text-xs uppercase tracking-[0.2em] text-slate-500">Total Spent</p>
            <p class="mt-3 text-3xl font-semibold text-emerald-600">&#8377;{{ number_format($totalSpent, 2) }}</p>
        </article>

        <article class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:col-span-2 lg:col-span-1">
            <p class="text-xs uppercase tracking-[0.2em] text-slate-500">Average Order Value</p>
            <p class="mt-3 text-3xl font-semibold text-sky-600">&#8377;{{ number_format($averageOrder, 2) }}</p>
        </article>
    </section>

    <section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-200 px-5 py-4">
            <h2 class="text-base font-semibold text-slate-900">Most Ordered Products</h2>
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-5 py-8 text-center text-slate-500">
                                No product order history found.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="2" class="px-5 py-8 text-center text-slate-500">
                                No orders found for status analysis.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
